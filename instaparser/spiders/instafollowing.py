...
class InstafollowingSpider(scrapy.Spider):
    ...
    def user_data_parse(self, response: HtmlResponse, username):
        ...
        inst_api_link = f'https://i.instagram.com/api/v1/friendships/{user_id}/following/?'
        count = 12
        url_following = f'{inst_api_link}count={count}'

        yield response.follow(url_following, callback=self.user_following_parse,
                              cb_kwargs={
                                  'username': username,
                                  'user_id': user_id,
                                  'count': count,
    ...
    def user_following_parse(self, response: HtmlResponse, username, user_id, count, inst_api_link):
        j_data = response.json()
        if j_data.get('next_max_id'):
            url_following = f"{inst_api_link}count={count}&max_id={j_data.get('next_max_id')}"
            yield response.follow(url_following, callback=self.user_following_parse,
                                  cb_kwargs={
                                      'username': username,
                                      'user_id': user_id,
